<template>
  <div class="input-text" v-if="!savedKey">
    ...
    <input
      ...
      @keydown.tab.exact.prevent="
        suggestions && suggest(suggestionsFiltered[0])
      "
      @keypress.enter="$emit('end')"
      @input="(ev) => $emit('update:value', ev?.target?.value)"
    />
    <div
      class="suggestions suggestions-action cursor-pointer flex flex-wrap justify-center mt-1"
      v-if="suggestions"
    >
...
export default defineComponent({
  ...
  computed: {
    suggestionsFiltered() {
      ...
      const byStart = this.suggestions.filter((el) =>
        this.value ? localeStart(el.name, this.value) : true
      );
      const byInclude = this.suggestions.filter((el) =>
        this.value ? localeIncludes(el.name, this.value) : true
      );
  ...
  methods: {
    ...
    checkSavedKey() {
      this.savedKey = window.localStorage.getItem("key") || "";
    },
  },
  mounted() {
    this.checkSavedKey();
