...
export default class SimpleGPT {
    ...
    async get(
        prompt: string,
        opts?: Partial<CreateCompletionRequest & CreateChatCompletionRequest>
    ): Promise<null | string[]> {
        ...
        const messages = opts?.messages || [
            { role: "user", content: _prompt as string },
        ];
        const response = await this._openai[
            isChatModel ? "createChatCompletion" : "createCompletion"
        ]({
            model,
            ...
            frequency_penalty:
                ...
                this.defaultOptsGPT.frequency_penalty,
            presence_penalty:
                opts?.presence_penalty || this.defaultOptsGPT.presence_penalty,
        });
        return (response?.data?.choices as any)
